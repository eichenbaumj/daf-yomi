/**
 * One bar that zooms. It opens on the six Orders with today marked (Joe, 2026-09-22:
 * one fewer step, and the labelled Orders make it look like the dynamic little bar it
 * is). Tap: this Order's tractates (1); tap: this tractate's dapim (2); tap: back out.
 * Every level is drawn in days, so each
...
export function renderPositionMini(ref: DafRef, learnedThroughDaf: number, lang: Lang = "en"): string {
  ...
  const regions = [[0, 1], [o0, o1], [t0, t1]];
  const sederName = sederShort(lang, t.seder, t.sederHe);
  const caps = [S.capTalmud, sederName, tractateName(lang, t)];
  const vals = [
  ...
  <div class="zoom-stage">
    <div class="zl on" data-l="0"><div class="geo">${orderGeo.join("")}</div><div class="labels">${orderLabels.join("")}</div></div>
    <div class="zl" data-l="1"><div class="geo">${tractGeo.join("")}</div><div class="labels">${tractLabels.join("")}</div></div>
    <div class="zl" data-l="2"><div class="geo dapim s${si + 1}">${cells.join("")}</div></div>
