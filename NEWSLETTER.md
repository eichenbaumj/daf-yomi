...
| Inline box | `renderInlineSubscribe` in `src/render/newsletterPages.ts`, under the AI note on every English daf page, a `<details>` folded to one line | Same POST with the choices collapsed (morning, the browser's zone via a hidden field, no hold); a link leads to the full form. Turnstile is rendered explicitly on first open (a widget rendered inside a closed `<details>` measures zero width), `interaction-only`, so it is invisible unless a challenge is needed. Shown only with `NEWSLETTER_PUBLIC=1` and a `TURNSTILE_SITE_KEY`; never on `/he`. Added 2026-09-21 after ~150 human visits produced 2 sign-ups with only a nav tab asking. |
