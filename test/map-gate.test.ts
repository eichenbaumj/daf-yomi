...
describe("map gate", () => {
  ...
  it("holds the caps", () => {
    expect(problems(withUnit(2, { title: "one two three four five six seven eight nine" }))).toContainEqual(expect.stringMatching(/title is 9 words; at most 8/));
    expect(problems(withUnit(2, { gloss: "w ".repeat(26).trim() }))).toContainEqual(expect.stringMatching(/gloss is 26 words/));
    ...
    expect(problems({ ...good, shape: "w ".repeat(40).trim() })).toContainEqual(expect.stringMatching(/shape sentence is 40 words/));
